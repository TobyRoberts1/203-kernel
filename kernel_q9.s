...
main:
    ...
    #create idle task for when nothing esle is running
    la $1, idle_pcb
    #sw $1, current_task($0)
    # Setup the link field
    la $2, idle_pcb
    ...
    #set task 3 as the previos task 
    la $1, task3_pcb
    sw $1, previous_task($0)
...
quit_tasks:
    ...
    #use the linked list 
    lw $1, previous_task($0)
    lw $2, current_task($0)

    #check if both tasks are the same meaning all quit 
    sequ $3, $2, $1
    bnez $3, idle
...
handle_time_slice:  
    lw $13, time_slice($0)      #store current time slice into $1
    subui $13, $13, 1
    sw $13, time_slice($0)
    beqz $13, dispatcher
...
schedule:
    ...
    sw $13, previous_task($0) #swap previous task
...
set_1_2_time:
    addui $13, $0, 1            #sets time sice to 1 if it is not the game. and saves in timeslice. 
    sw $13,time_slice($0)
...
playing_game:           
    ...
    bnez $13, load_context          #jumps back to loading context if not equal to 0
...
#sets up for the idle task and loads the pcb
idle: 
    la $3, idle_pcb
    sw $3, current_task($0)
    j load_context
...
#keeps it stuck in an infinite loop 
idle_loop:
    j idle_loop
